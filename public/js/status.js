$(document).ready(function() { 
  // ---------------  Request data ---------------------------
  $(".jumbotron").hide();

  $.getJSON( "http://" + SERVER + ":" + NODEPORT + "/status_raw").done(function(data) {
    $("#total-files h3").append(data.length);
    $('#loadingDiv').hide();
    $(".jumbotron").show();
    var noSqlFiles = true;
    if (data.length == 0 ) {
      $(".status_content").append("<p style='margin-left: 45%; margin-top:17%;'>No Changes</p>");
    }
    else { 
      for (var i = 0; i < data.length; i++) {
        if(data[i].toString().search("sql+") > -1) {
          noSqlFiles = false;
          $(".status_content").append("<ul style='margin-right: 45%;'><li style='cursor: pointer;' data-toggle='modal' data-target='#gitdiff-dialog' id='file_" + i + "'>" + data[i].toString() + "</li></ul>")
      
          if(data[i].toString().search("^\\sD\\s") > -1) { 
            $("#file_" + i).css("background-color", "#FF5252");
          }
          else {
            if(data[i].toString().search("^\\sM\\s") > -1) { 
              $("#file_" + i).css("background-color", "#FFC107");
            } 
            else { 
              $("#file_" + i).css("background-color", "#CDDC39");
            }
          }                                      
        }
      }
      if(noSqlFiles) {
        $(".status_content").append("<p style='margin-left: 45%; margin-top:17%;'>No Changes</p>");
      }
    }  

    $('.status_content').on( 'click', 'li', function () {
      $(".modal-body").html("");
      var fileName = $(this).text();
      fileName = fileName.replace(new RegExp("^..."),"");
      $("#loadingDiv").show();
      $.getJSON("http://" + SERVER + ":" + NODEPORT + "/differences?file=" + fileName, function(data) {
        $.when(data).done(function() {
          $('#loadingDiv').hide();
          for (var i = 0; i < data.length; i++) {
            if (data[i].search("^\\-") > -1 ) { $(".modal-body").append("<b><p style='color: red;'>" + data[i] + "</p></b>"); }
            else { 
              if (data[i].search("^\\+") > -1 ) { $(".modal-body").append("<b><p style='color: green;'>" + data[i] + "</p></b>"); }
              else {
                $(".modal-body").append("<p>" + data[i] + "</p>");
              }
            }    
          }
        })
      });    
    });
  })
  .fail(function () {
    $("#myTabContent").prepend('<div class="alert alert-dismissible alert-danger"><button type="button" class="close" data-dismiss="alert">×</button><strong><p>Oops..!</p></strong> <p>Something went wrong...</p></div>');
  });
});

